...
const tenantId = config.tenantId || "vitorino-doces-finos";
...
} else if (hasFirebaseConfig(firebaseConfig)) {
  init();
}

async function init() {
  try {
    const app = initializeApp(firebaseConfig, "vitorino-public");
    ...
    await loadCustomScripts(landing);
...
function hasFirebaseConfig(value) {
  return Boolean(value?.apiKey && value?.projectId && value?.appId);
}

async function fetchLanding() {
  const reference = doc(state.db, "tenants", tenantId, "landingPages", landingSlug);
  const snapshot = await getDoc(reference);
  if (!snapshot.exists()) return null;
  const landing = normalizeRecord({ id: snapshot.id, ...snapshot.data() });
...
function text(selector, value, preserveStrong) {
  ...
  if (preserveStrong) {
    ...
    const strong = strongElement?.outerHTML || "";
    const copy = String(value).replace(strongElement?.textContent || "", "").trim();
...
async function loadCustomScripts(landing) {
  if (!landing?.id || !state.db) return;
  try {
    const snapshot = await getDoc(doc(state.db, "tenants", tenantId, "landingScripts", landingSlug));
    if (!snapshot.exists()) return;
    const scripts = snapshot.data();
    if (!scripts?.enabled) return;
...
function executableScript(node) {
  ...
  [...node.attributes].forEach((attrItem) => script.setAttribute(attrItem.name, attrItem.value));
...
function track(eventName, details = {}) {
  ...
  if (!enabled || !state.db || !state.landing?.id || !hasConsent(state.landing.settings || {})) return;
  const payload = {
    tenant_id: tenantId,
  ...
  addDoc(collection(state.db, "tenants", tenantId, "trackingEvents"), payload).catch(() => {});
}
